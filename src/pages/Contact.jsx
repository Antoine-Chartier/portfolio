...
const Contact = () => {
  ...
  return (
    <div className="clamp contact-page"> {/* Added class for page styling */}
      ...
      <AnimatePresence mode="wait">
        {submissionStatus !== 'success' ? (
          ...
          >
            <h2>Contactez-moi</h2> {/* Added a title */}
            <form
              name="contact"
              method="post"
              data-netlify="true"
            ...
            {submissionStatus === 'error' && (
              <motion.p className="error-message" initial={{opacity: 0}} animate={{opacity: 1}}>Oops! Une erreur est survenue. Veuillez réessayer.</motion.p>
            )}
          </motion.div>
        ) : (
          // Show Success Message
          <motion.div key="success" className="success-message" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}>
            <h2>Merci !</h2>
            <p>Votre message a bien été envoyé. Je reviendrai vers vous rapidement.</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
